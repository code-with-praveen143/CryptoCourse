...
const Coin = () => {
  ...
  // Fetch coin price and performance data
  const fetchCoinData = async () => {
    ...
    try {
      ...
      setPrice(priceData.price); // Assuming API returns { price: value }
      ...
      setText(infoData.description); // Assuming API returns { description: "text" }
      setChangeH(infoData.change.hour);
      setChangeD(infoData.change.day);
      setChangeW(infoData.change.week);
    } catch (error) {
      console.error("Error fetching coin data:", error);
    }
  };

  // Fetch user balance for the coin
  const fetchBalance = async () => {
    try {
      const response = await fetch("http://localhost:5000/user/balance", {
        ...
        body: JSON.stringify({ username: localStorage.getItem("username") }), // Replace with actual username
      });
      const data = await response.json();
      setCurrentUSD(data.balance);
      setCurrentCoin(coin ? data[coin as keyof typeof data] || 0 : 0); // Use coin to fetch specific balance
    } catch (error) {
  ...
  const handleBuy = async () => {
    try {
      ...
      if (response.ok) {
        await fetchBalance(); // Refresh balances after buying
      } else {
  ...
  const handleSell = async () => {
    try {
      ...
      if (response.ok) {
        await fetchBalance(); // Refresh balances after selling
      } else {
        console.error("Sell action failed");
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    fetchCoinData();
    fetchBalance();
    setLogo(coinLogos[coin?.toLowerCase()] || ""); // Set coin logo based on coin name
  }, [coin]);
        ...
        <div className="bg-gray-800 rounded-lg p-6 shadow-lg text-white">
          ...
          <div className="border border-gray-700 rounded-md p-4 mb-6">
            <h3 className="text-xl font-semibold mb-4 text-center">
              {coin?.toUpperCase()} in Your Portfolio
            </h3>
          ...
          <div className="border border-gray-700 rounded-md p-4 mb-6">
            ...
            <div className="space-y-2 text-lg">
              <div className="flex justify-between">
                ...
                <span
                  className={changeH >= 0 ? "text-green-500" : "text-red-500"}
                >
                  {changeH}%
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span
                  className={changeD >= 0 ? "text-green-500" : "text-red-500"}
                >
                  {changeD}%
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span
                  className={changeW >= 0 ? "text-green-500" : "text-red-500"}
                >
                  {changeW}%
                </span>
              </div>
          ...
          <div className="border border-gray-700 rounded-md p-4">
            ...
            <div className="text-center text-2xl font-bold text-green-500 mb-6">
              ${price}
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between mt-6 space-y-4 sm:space-y-0 sm:space-x-4">
              <button
                className="w-full sm:w-auto p-3 bg-green-600 rounded text-white hover:bg-green-700"
                onClick={handleBuy}
              >
                Buy
              </button>
              <button
                className="w-full sm:w-auto p-3 bg-red-600 rounded text-white hover:bg-red-700"
                onClick={handleSell}
              >
                Sell
              </button>
